...
-- 부서번호 d005 부서의 현재 관리자 벙보를 출력하세요.
...
/*
 ...
 - 테이블 내에 존재하지 않는 조건에 근거한 값들을 검색할 때 사용.
 */
...
with current as (select emp_no
                 from dept_mamager -- 부서별 관리자 tb
                 where to_date = '9999-01-01' -- 재직자
                   and dept_no = 'd005');
select *
from employees
...
from dept_emp
where to_date = '9999-01-01';
-- employees db 에서 재직자의 평균 그명를 출력하시오.
select avg(salary)
from salaries
where to_date = '9999-01-01';
...
-- 부서 번호로 구분하고, 부서 번호로 올므 차순 정렬하여 출력한다.
